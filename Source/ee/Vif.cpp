    ...
    , m_vifProfilerZone(CProfiler::GetInstance().RegisterZone(string_format("VIF%d", number).c_str()))
{
	loop<int, 0x100>(
	    [this](auto i) {
		    ...
		    m_unpacker[i] = &CVif::UnpackGeneric<dataType, clGreaterEqualWl, useMask, mode>;
	    });
}
...
void CVif::Cmd_UNPACK(StreamType& stream, CODE nCommand, uint32 nDstAddr)
{
	...
	auto unpackFct = m_unpacker[(nCommand.nCMD & 0x0F) | ((clGreaterEqualWl ? 1 : 0) << 4) | ((useMask ? 1 : 0) << 5) | (mode << 6)];
